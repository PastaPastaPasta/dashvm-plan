...
function renderInspector() {
  ...
  const pr = t.pr ? `${prLink(t)} ${t.pr.draft ? "(draft)" : ""}<br><span class="secondary">CI ${t.pr.ci_green === true ? "green" : t.pr.ci_green === false ? "not green" : "unknown"}${t.pr.ci_ignored?.length ? ` (ignoring ${esc(t.pr.ci_ignored.join(", "))})` : ""} · thepastaclaw ${t.pr.pastaclaw_approved ? "approved" : "not yet"} · unresolved threads ${t.pr.unresolved ?? "?"}${t.pr.gate ? " · gate: " + esc(t.pr.gate) : ""}</span>` : "none";
